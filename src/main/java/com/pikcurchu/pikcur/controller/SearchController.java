...
public class SearchController {
    ...
    @GetMapping
    public ResGoodsPageDto selectSearchGoodsList(@RequestParam String keyword, HttpServletRequest request,
            @RequestParam int currentPage) {
        Integer memberNo = (Integer) request.getAttribute("memberNo");
        return searchService.selectSearchGoodsList(keyword, memberNo, currentPage);
    }

    @Operation(summary = "검색어 히스토리 조회", description = "회원 번호를 통한 최근 검색어 히스토리 리스트 조회")
    @GetMapping("/recent")
    public List<ResSearchDto> selectSearchHistory(HttpServletRequest request) {
        Integer memberNo = (Integer) request.getAttribute("memberNo");
        return searchService.selectSearchHistory(memberNo);
    }

    @Operation(summary = "인기 검색어 조회", description = "인기 검색어 top10 리스트 조회")
    @GetMapping("/popular")
    public List<ResSearchDto> selectSearchTop10() {
        return searchService.selectSearchTop10();
    }

    @Operation(summary = "검색어 히스토리 삭제", description = "최근 검색어 삭제")
    @DeleteMapping("/recent/{searchHistoryId}")
    public void deleteSearchHistory(HttpServletRequest request, @PathVariable Integer searchHistoryId) {
        Integer memberNo = (Integer) request.getAttribute("memberNo");
        searchService.deleteSearchHistory(memberNo, searchHistoryId);
    }
}
